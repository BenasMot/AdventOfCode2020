...
const isValid = (passport: Data): passport is ValidPassport => {
  ...
  if (hasFields) {
    ...
    const validHCL =
      passport.hcl[0] === "#" &&
      isHexEightBit(passport.hcl.slice(1)) &&
      passport.hcl.length - 1 === 6;
...
const countAcceptablePassports = (data: Data[]) => {
  const count = data.filter(hasRequiredFields).length;
  return count;
};

const countValidPassports = (data: Data[]) => {
...
const part1 = (rawInput: string) => {
  ...
  return countAcceptablePassports(input);
};
...
run({
  part1: {
    tests: [{ input: readFileSync("./src/day04/input.txt", "utf8"), expected: 202 }],
    solution: part1,
  },
  part2: {
    tests: [{ input: readFileSync("./src/day04/input.txt", "utf8"), expected: 137 }],
    solution: part2,
  },
  trimTestInputs: true,
  onlyTests: false,
